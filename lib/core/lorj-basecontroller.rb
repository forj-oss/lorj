...
module Lorj
  class BaseController
    ...
    def connect(_sObjectType, _hParams)
      fail Lorj::PrcError.new, "connect has not been redefined by the controller '%s'" % self.class
    end

    # Default handlers which needs to be defined by the controller,
    # called by BaseDefinition Create functions.
    def create(_sObjectType, _hParams)
      fail Lorj::PrcError.new, "create_object has not been redefined by the controller '%s'" % self.class
    end

    # Default handlers which needs to be defined by the controller,
    # called by BaseDefinition Delete functions.
    def delete(_sObjectType, _hParams)
      fail Lorj::PrcError.new, "delete_object has not been redefined by the controller '%s'" % self.class
    end

    # Default handlers which needs to be defined by the controller,
    # called by BaseDefinition Get functions.
    def get(_sObjectType, _sUniqId, _hParams)
      fail Lorj::PrcError.new, "get_object has not been redefined by the controller '%s'" % self.class
    end

    # Default handlers which needs to be defined by the controller,
    # called by BaseDefinition Query functions.
    def query(_sObjectType, _sQuery, _hParams)
      fail Lorj::PrcError.new, "query_object has not been redefined by the controller '%s'" % self.class
    end

    # Default handlers which needs to be defined by the controller,
    # called by BaseDefinition Update functions.
    def update(_sObjectType, _oObject, _hParams)
      fail Lorj::PrcError.new, "update_object has not been redefined by the controller '%s'" % self.class
    end
    ...
    #  - Lorj::PrcError
    def Error(msg)
      fail Lorj::PrcError.new, '%s: %s' % [self.class, msg]
    end
    ...
    def required?(oParams, *key)
      fail Lorj::PrcError.new, '%s: %s is not set.' % [self.class, key] unless oParams.exist?(key)
    end
  end
end
