...
module Lorj
  ...
  class Core
    ...
    def initialize(oForjConfig = nil, processesClass = nil, sControllerClass = nil)
      ...
      if processesClass.nil?
        aProcessesClass = []
      elsif !processesClass.is_a?(Array)
      ...
      aProcessesClass.each do | sProcessClass |
        ...
        if File.exist?(sFile)
          ...
          sProcessClass = '%sProcess' %  sProcessClass unless /Process$/ =~ sProcessClass
      ...
      if sControllerClass
        ...
        if File.exist?(sFile)
          ...
          fail Lorj::PrcError.new, "Provider file definition '%s' is missing. Cannot go on" % sFile
        end
      ...
      unless cProcessClass.nil?
        ...
        fail Lorj::PrcError.new, 'Lorj::Core: No valid process loaded. Aborting.'
      end
    ...
    def Connect(oCloudObj, hConfig = {})
      return nil if !oCloudObj || !@oCoreObject
    ...
    def Get(oCloudObj, sId, hConfig = {})
      return nil if !oCloudObj || !@oCoreObject || sId.nil?
  ...
  class CloudCore < Core
    def initialize(oConfig, sAccount = nil, aProcesses = [])
      ...
      fail Lorj::PrcError.new, 'Provider_name not set. Unable to create instance CloudCore.' if sControllerMod.nil?
  ...
  class ObjectData
    ...
    def add(oDataObject)
      ...
      fail Lorj::PrcError.new, "Invalid Framework object type '%s'." % oDataObject.class unless oDataObject.is_a?(Lorj::Data)
    ...
    def delete(oObj)
      if oObj.is_a?(Symbol)
        ...
        fail Lorj::PrcError.new, "ObjectData: delete error. oObj is not a framework data Object. Is a '%s'" % oObj.class unless oObj.is_a?(Lorj::Data)
        if oObj.type? == :list
    ...
    #   PrcError
    def exist?(*key)
      fail Lorj::PrcError.new, 'ObjectData: key is not list of values (string/symbol or array)' unless [Array, String, Symbol].include?(key.class)
      ...
      if oObject.is_a?(Lorj::Data)
        ...
        return true if key[1] == :ObjectData && oObject.type?(key[0]) == :object
