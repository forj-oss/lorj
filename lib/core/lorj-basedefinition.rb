  ...
  class BaseDefinition
    ...
    def Setup(sObjectType, sAccountName)
      ...
      while oInspectObj.length >0
        ...
        hTopParams[:keys].each do |sKeypath, hParams|
          ...
          case hParams[:type]
             when :data
               ...
               next if hMeta.nil?
               sAsk_step = hMeta[:ask_step] if Lorj.rhExist?(hMeta, :ask_step) == 1 && hMeta[:ask_step].is_a?(Fixnum)
               ...
               if hMeta[:account].is_a?(TrueClass)
                 if not hMeta[:depends_on].is_a?(Array)
                   ...
                   bFound = false
                   iLevel = 0
                   hMeta[:depends_on].each do |depend_key|
                     aOrder.each_index do |iCurLevel|
      ...
      hAskStep.each_index do | iStep |
        value = hAskStep[iStep]
        if Lorj.rhExist?(value, :add) == 1
          sKeysToAdd = Lorj.rhGet(value, :add)
          sKeysToAdd.each do | sKeyToAdd |
            bFound = false
            aSetup[iStep][:order].each_index do | iOrder |
            ...
            if Lorj.rhExist?(hMeta, :after) == 1
              ...
              sAfterKeys.each do |sAfterKey |
                bFound = false
                aSetup.each_index do |iStepToCheck|
                  aSetup[iStepToCheck][:order].each_index do | iLevelToCheck |
      ...
      aSetup.each_index do | iStep |
        ...
        aOrder.each_index do | iIndex |
          Lorj.debug(2, "Ask order %s:" % iIndex)
          aOrder[iIndex].each do | sKey |  
            ...
            while not bOk  
              ...
              if not hParam[:list_values].nil?  
                ...
                case hValues[:query_type]  
                   ...
                   when :process_call  
                     ...
                     unless hValues[:query_params].nil?  
                       hValues[:query_params].each do | key, value |  
                     ...
                     if hProcResult.is_a?(Hash)  
                       ...
                       if hProcResult[:list].nil? || ! hProcResult[:list].is_a?(Array)  
                ...
                PrcLib.fatal(1, "%s requires a value from the '%s' query which is empty." % [sKey, sObjectToLoad])if aList.nil? && bListStrict  
                aList = [] if aList.nil?  
                if not bListStrict  
                  aList << "other"  
                end  
    ...
    def register(oObject, sObjectType = nil, sDataType = :object)
      if oObject.is_a?(Lorj::Data)
        ...
        oDataObject.set(oObject, sObjectType) do | sObjType, oControlerObject |
    ...
    def connect(sObjectType)
      ...
      oDataObject.set(oControlerObject, sObjectType) do | sObjType, oObject |
        begin
    ...
    def query(sObjectType, hQuery)
      ...
      oDataObjects.set(oControlerObject, sObjectType, hQuery) do | sObjType, key |
        begin
    ...
    def update(sObjectType)
      ...
      bUpdated = false
      oObject[:attrs].each do |key, value |
        oKeyPath = KeyPath.new(key)
      ...
      oObject.set(oControlerObject, sObjectType) do | sObjType, oObject |
        begin
          _return_map(sObjType, oObject)
       rescue => e
    ...
    def cloud_obj_requires(sCloudObj, res = {})
      ...
      Lorj.rhGet(@@meta_obj,sCloudObj, :params).each do |key, hParams|
        case hParams[:type]
           when :data
             if  hParams.key?(:array)
               hParams[:array].each do | aElem |
                 ...
                 Lorj.rhGet(hParams, aElem).each do | subkey, _hSubParam |
                   next if aElem.length == 0 && [:array, :type].include?(subkey)
                   if hSubParams[:required] && @oForjConfig.get(subkey).nil?
                     res[subkey] = hSubParams
                   end
                 end
               end
             else
               if hParams[:required] && @oForjConfig.get(key).nil?
                 res[key] = hParams
               end
             end
           when :CloudObject
    ...
    def get_forjKey(_oCloudData, key)
      return nil if not @ObjectData.exist?(sCloudObj)
