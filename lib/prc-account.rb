...
module Lorj
  class Accounts
    ...
    def dump
      aAccounts = []
      Dir.foreach($FORJ_ACCOUNTS_PATH) { |x| aAccounts << x unless x.match(/^\..?$/) }
      aAccounts
    end
  end
  ...
  class Account
    ...
    def metadata_each
      Lorj.rhGet(Lorj::Default.dump, :sections).each do | section, hValue |
    ...
    def set(key, value)
      ...
      return @oConfig.set(key, value) unless section
    ...
    def ac_save
      ...
      unless @oConfig.localDefaultExist?('account_name')
