...
module Lorj
  ...
  def self.rhKeyToSymbol(yVal, levels = 1)
    return nil if yVal.nil? || yVal.class != Hash
    yRes = {}
    yVal.each do | key, value |
      if key.class == String
        if levels <= 1
  ...
  #   Nothing
  def self.rhKeyToSymbol?(yVal, levels = 1)
    return false if yVal.nil? || yVal.class != Hash
    yVal.each do | key, value |
      if key.class == String
        return true
      end
  ...
  class Default
    ...
    def self.meta_each
      ...
      Lorj.rhGet(@@yDefaults, :sections).each do | section, hValue |
  ...
  class Config
    ...
    def default_dump(interms = nil)
      ...
      res = {}
      Lorj::Default.dump[:default].each_key do |key|
        dump_key = exist?(key)
        Lorj.rhSet(res, get(key), dump_key, key)
      end
      if Lorj.rhExist?(@yLocal, :default) == 1
        @yLocal[:default].each_key do |key|
          dump_key = exist?(key)
      ...
      if interms
        if interms.instance_of? Hash
          @interms.each_key do | key|
            dump_key = exist?(key)
            Lorj.rhSet(res, get(key), dump_key, key) if Lorj.rhExist?(res, dump_key, key) != 2
          end
        elsif interms.instance_of? Array # Array of hash of hash
          interms.each do | elem |
            elem.each_key do | key|
              dump_key = exist?(key)
              Lorj.rhSet(res, get(key), dump_key, key) if Lorj.rhExist?(res, dump_key, key) != 2
            end
          end
    ...
    def set(key, value)
      ...
      return false unless [Symbol, Array].include?(key.class)
    ...
    def get(key, default = nil)
      key = key.to_sym if key.class == String
      return nil unless [Symbol, Array].include?(key.class)
    ...
    def exist?(key, interms = nil)
      ...
      if interms
        if interms.instance_of? Hash
          ...
          interms.each do | elem |
            if elem.class == Hash
              elem.each do | hashkey, value |
                return ("%s" % hashkey)  if value.class == Hash && Lorj.rhExist?(elem, hashkey, key) == 2
                return ("hash[%s]" % iCount)  if value.class != Hash && Lorj.rhExist?(elem, hashkey) == 1
              end
            end
            iCount += 1
          end
    ...
    #   - ++ ->
    def meta_each
      Lorj::Default.meta_each do |section, key, value|
