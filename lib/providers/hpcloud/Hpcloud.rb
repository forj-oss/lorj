...
class HpcloudController
  ...
  def get_attr(oControlerObject, key)
    if oControlerObject.is_a?(Excon::Response)
      ...
      fail "attribute '%s' is unknown in '%s'. Valid one are : '%s'" % [key[0], oControlerObject.class, oControlerObject.class.attributes] unless oControlerObject.class.attributes.include?(key[0])
  ...
  def set_attr(oControlerObject, key, value)
    fail "No set feature for '%s'" % oControlerObject.class if oControlerObject.is_a?(Excon::Response)
    attributes = oControlerObject.attributes
    fail "attribute '%s' is unknown in '%s'. Valid one are : '%s'" % [key[0], oControlerObject.class, oControlerObject.class.attributes] unless oControlerObject.class.attributes.include?(key[0])
